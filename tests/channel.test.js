...
test('최근 업데이트 목록', async () => {
  const channelList = await channelDao.findAllChannelList(14)
...
test('채널 생성 시 isPublic 속성 확인', async () => {
  ...
  // Create the channel
  const createdChannel = await channelDao.create(testChannelData);
  ...
  const retrievedChannel = await channelDao.findOneByChannelId(testChannelId);
  ...
  const deletedChannel = await channelDao.findOneByChannelId(testChannelId);
...
test('findAndCountAllVideo public/private filtering', async () => {
  ...
  await accountDao.createAccount({ accountId: testAccountId1, username: 'User1' });
  await accountDao.createAccount({ accountId: testAccountId2, username: 'User2' });

  // Create test channels
  const channelPublic1 = await channelDao.create({
  ...
  let result = await videoDao.findAndCountAllVideo('dev', 'en', 0, 10, '', null, null, null);
  ...
  result = await videoDao.findAndCountAllVideo('dev', 'en', 0, 10, '', null, null, testAccountId1);
  ...
  result = await videoDao.findAndCountAllVideo('dev', 'en', 0, 10, '', null, null, testAccountId2);
  channelTitles = result.rows.map(video => video.Channel.title);
  expect(channelTitles).toContain('Public Channel 1');
  expect(channelTitles).toContain('Public Channel 2');
  expect(channelTitles).not.toContain('Private Channel 1');
